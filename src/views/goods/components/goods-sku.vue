<template>
  <div class="goods-sku">
    <dl v-for="item in goods.specs" :key="item.sku_code">
      <dt>{{item.name}}</dt>
      <dd>
    ...
    if (sku.inventory > 0) {
      ...
      valueArrPowerSet.forEach(arr => {
       ...
       const key = arr.join(spliter)
        // 给pathMap设置数据
        if (pathMap[key]) {
          pathMap[key].push(sku.sku_code)
        } else {
          pathMap[key] = [sku.sku_code]
        }
      })
    }
  })
...
const initDefaultSelected = (goods, skuId) => {
  ...
  const sku = goods.skus.find(sku => sku.sku_code === skuId)
  ...
  setup(props, { emit }) {
    ...
    const changeSku = (item, val) => {
      ...
      if (validSelectedValues.length === props.goods.specs.length) {
        ...
        const sku = props.goods.skus.find(sku => sku.sku_code === skuIds[0])
